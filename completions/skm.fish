...
complete -c skm -n "__fish_skm_needs_command" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand init" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand import" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and not __fish_seen_subcommand_from setup extend ls show rm help" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and __fish_seen_subcommand_from setup" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and __fish_seen_subcommand_from extend" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and __fish_seen_subcommand_from ls" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and __fish_seen_subcommand_from show" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand profile; and __fish_seen_subcommand_from rm" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand skill; and not __fish_seen_subcommand_from ls setup rm validate help" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand skill; and __fish_seen_subcommand_from ls" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand skill; and __fish_seen_subcommand_from setup" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand skill; and __fish_seen_subcommand_from rm" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand skill; and __fish_seen_subcommand_from validate" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand use-profiles" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand add-profile" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand remove-profile" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand use-agents" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand add-agent" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand remove-agent" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand destroy" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand sync" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand status" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand ls" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand scan" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand search" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand doctor" -l json -d 'Emit machine-readable JSON on stdout (`status`, `ls`, `search`, `skill ls`, `skill validate`, `doctor` only)'
...
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "init" -d 'Set up the skill store and project config (`.skm.toml`)'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "import" -d 'Import a skill directory into the store'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "profile" -d 'Create and manage profiles'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "skill" -d 'Manage skills in the store'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "use-profiles" -d 'Choose active profiles and sync skill links (interactive)'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "add-profile" -d 'Add a profile to the active set and sync skill links'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "remove-profile" -d 'Remove a profile from the active set and sync skill links'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "use-agents" -d 'Choose target agents interactively'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "add-agent" -d 'Add a target agent to this setup'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "remove-agent" -d 'Remove a target agent from this setup'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "destroy" -d 'Tear down this project\'s skm setup (`.skm.toml` and store-owned links)'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "sync" -d 'Refresh skill links without changing the active profiles'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "status" -d 'Show target agents, active profiles, linked skills, and name conflicts'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "ls" -d 'List skills and profiles in the store'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "scan" -d 'Refresh the on-disk skill index'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "search" -d 'Search indexed skill IDs and descriptions'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "doctor" -d 'Read-only health report for the store, profiles, and skill links'
complete -c skm -n "__fish_skm_using_subcommand help; and not __fish_seen_subcommand_from init import profile skill use-profiles add-profile remove-profile use-agents add-agent remove-agent destroy sync status ls scan search doctor help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
